        ...
        try:
            error_element = WebDriverWait(driver, 10).until(
                EC.visibility_of_element_located((
                    By.XPATH,
                    "//input[@id='phone']/following-sibling::p[contains(@class, 'text-red-600')]"
                ))
            )
            
            assert "Phone number must be 10-15 digits" in error_element.text
            assert error_element.find_element(By.TAG_NAME, "svg").is_displayed()
            print("✔ Phone validation error displayed correctly")
            ...
            try:
                ...
                raise AssertionError("Form submitted with invalid phone number")
            except:
                print("✔ Form submission blocked as expected")

            print("✅ Test Passed: Phone number validation working correctly")

        except Exception as e:
            print(f"❌ Test Failed: {str(e)}")
            driver.save_screenshot("phone_validation_failure.png")
