    ...
    <div className="max-w-3xl mx-auto mt-10 p-6 bg-gray-50 shadow-lg rounded-xl">
      ...
      <div className="space-y-3 text-lg text-gray-800">
        ...
        {data.itemNames && data.itemNames.length > 0 && (
          <p><strong>Item Names:</strong> {data.itemNames.join(', ')}</p>
        )}
        <p><strong>Quantity:</strong> {data.quantity} {data.unit}</p>
        <p><strong>Price per Unit (Rs):</strong> {data.price}</p>
      ...
      <button onClick={() => navigate('/')} className="mt-6 w-full bg-blue-600 text-white py-2 rounded-lg hover:bg-blue-700">
        Add Another Order
      </button>
    </div>
  );
